...
class DynamicCommandCompleter(Completer):
    def __init__(self):
        ...
        self.commands = {
            "/theme": ["--style", "--list", "--help"],
            "/sysinfo": ["--g", "--cpu", "--ram", "--disk", "--display", "--input", "--help"],
            "/system": ["--taskmgr", "--end-task", "--run-new", "--d", "--e"],
            "/help": [],
            "/quit": [],
            "/clear": []
        }
                ...
                for item in os.listdir(functions_dir):
                    ...
                    if os.path.isdir(item_path) and not item.startswith("__"):
                        ...
                        if cmd not in self.commands:
                            self.commands[cmd] = ["--help"] # Default flag for discovered commands
        except Exception:
            pass
    ...
    def get_completions(self, document: Document, complete_event):
        ...
        if cmd in self.commands:
            ...
            prev_word = parts[-1] if text.endswith(" ") else (parts[-2] if len(parts) > 1 else "")
