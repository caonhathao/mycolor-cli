...
def handle_help_command(log_to_buffer):
    ...
    log_to_buffer("")
    log_to_buffer(
        f"[bold {primary_hex}]--- Configuration (settings.json) ---[/bold {primary_hex}]"
    )
    log_to_buffer("")
    ...
    log_to_buffer(
        f"[{table_text}]show_system_processes[/{table_text}] : (bool) Toggle OS tasks in /system ({show_system})"
    )
    log_to_buffer(
        f"[{table_text}]buffer_limit[/{table_text}]           : (int) Max history lines (2,500)"
    )
    log_to_buffer(
        f"[{table_text}]theme[/{table_text}]                   : (str) Active UI theme ({theme})"
    )
    log_to_buffer(
        f"[{table_text}]default_export_path[/{table_text}]  : (str) Default for /copy --export"
    )
    log_to_buffer(
        f"[{table_text}]process_update_interval[/{table_text}] : (float) Task Manager refresh ({interval}s)"
    )
    log_to_buffer("")
    log_to_buffer(
        "[bold #00FF88]Tip: Use <command> --help for detailed flag descriptions (e.g., /system --help).[/bold #00FF88]"
    )
